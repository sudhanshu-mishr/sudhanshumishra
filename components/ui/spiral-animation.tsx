...
class AnimationController {
    ...
    public rotate(v1: Vector2D, v2: Vector2D, p: number, orientation: boolean): Vector2D {
        if (!v1 || !v2) return new Vector2D(0, 0);
...
class Star {
    ...
    render(p: number, controller: AnimationController) {
        ...
        if (q > 0) {
            ...
            let sizeMultiplier = displacementProgress < 0.6 
                ? 1.0 + displacementProgress * 0.2 
                : 1.2 * (1.0 - (displacementProgress - 0.6) / 0.4) + this.finalScale * ((displacementProgress - 0.6) / 0.4);
