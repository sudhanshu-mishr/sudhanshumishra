...
export const HorizonHeroSection = () => {
  ...
  useEffect(() => {
    const initThree = () => {
      ...
      // getLocation must be called after mountains are added
      const locations = refs.mountains.map(m => m.position.z);
    ...
    const animate = () => {
      ...
      if (refs.composer && refs.renderer) refs.composer.render();
    };
    ...
    return () => {
      ...
      refs.stars.forEach(sf => { if (sf) { sf.geometry.dispose(); (sf.material as THREE.Material).dispose(); } });
      refs.mountains.forEach(m => { if (m) { m.geometry.dispose(); (m.material as THREE.Material).dispose(); } });
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const progress = isNaN(rawProgress) ? 0 : Math.min(Math.max(rawProgress, 0), 1);
      ...
      const sectionIndex = Math.min(newSection, cameraPositions.length - 1);
      const currentPos = cameraPositions[sectionIndex] || cameraPositions[0];
      const nextPos = cameraPositions[Math.min(sectionIndex + 1, cameraPositions.length - 1)] || currentPos;
      ...
      if (refs.nebula && refs.mountains.length > 3 && refs.mountains[3]) {
  ...
  return (
    <div ref={containerRef} className="hero-container cosmos-style">
      ...
      <div ref={menuRef} className="side-menu">
        ...
        <div className="vertical-text">SYNTHESIS</div>
      </div>
      ...
      <div className="hero-content cosmos-content">
        ...
        <div ref={subtitleRef} className="hero-subtitle cosmos-subtitle">
          <p className="subtitle-line">Where vision meets silicon reality,</p>
          <p className="subtitle-line">synthesizing the architectures of tomorrow.</p>
        </div>
      </div>

      <div ref={scrollProgressRef} className="scroll-progress">
        <div className="scroll-text">SYSTEM_SCAN</div>
        <div className="progress-track">
          <div className="progress-fill" style={{ width: `${scrollProgress * 100}%` }} />
        </div>
        <div className="section-counter">
          {String(Math.min(currentSection + 1, totalSections + 1)).padStart(2, '0')} / {String(totalSections + 1).padStart(2, '0')}
        </div>
      </div>

      <div className="scroll-sections">
        {[...Array(totalSections)].map((_, i) => {
          const titles = ["SILICON ARCHITECT", "DIGITAL SYNTHESIS"];
          const subtitles = [
